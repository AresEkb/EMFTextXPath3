...
public class Xpath3DefaultNameProvider implements org.emftext.language.xpath3.resource.xpath3.IXpath3NameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.xpath3.resource.xpath3.util.Xpath3EObjectUtil.invokeOperation(element, operation);
					if (result != null) {
