...
/**
 * The Xpath3TaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * Xpath3TaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class Xpath3TaskItemDetector {
	...
	public List<org.emftext.language.xpath3.resource.xpath3.mopp.Xpath3TaskItem> findTaskItems(String text, int line, int charStart) {
		List<org.emftext.language.xpath3.resource.xpath3.mopp.Xpath3TaskItem> foundItems = new ArrayList<org.emftext.language.xpath3.resource.xpath3.mopp.Xpath3TaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.xpath3.resource.xpath3.mopp.Xpath3TaskItem(keyword, message, localLine, offset, end));
